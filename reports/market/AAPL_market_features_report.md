# AAPL: market feature table

One row per article: the label a model is trained against, and the market state as it stood at publication. Produced by `stock_predictor.market.run_pipeline`, which writes the table itself to `data/processed/market_run/AAPL/`. Joins the text layer's `article_features.parquet` on `article_id`.

**Rows:** 7,527 | **Columns:** 16 | **Span:** 2025-09-01 to 2026-08-01 | **Benchmark:** SPY
...
| majority-class rate | 50.9% |
| mean abnormal return | +0.0003 |
| std abnormal return | 0.0190 |


![AAPL label balance](figures/AAPL_market_label_balance.png)
...
| lowest monthly up-share | 31.9% (2025-12, 480 articles) |
| highest monthly up-share | 60.7% (2025-09, 422 articles) |
| spread | 28.8% |


![AAPL label balance by month](figures/AAPL_market_label_balance_monthly.png)
...
![AAPL abnormal return distribution](figures/AAPL_market_return_distribution.png)
...
![AAPL session counts](figures/AAPL_market_session_counts.png)
...
| metric | value |
| --- | --- |
| articles in corpus | 7,527 |
| labelled rows | 7,527 (100.0%) |
| date span | 2025-09-01 to 2026-08-01 (335 days) |
| active days | 328 of 335 (97.9%) |
| longest gap | 2 consecutive days with no article |
| articles/day | min 0, median 18, max 94 |


![AAPL market features per month](figures/AAPL_market_monthly.png)

![AAPL market features per day](figures/AAPL_market_daily.png)
...
|---|---|---:|---:|---:|---|
| `abnormal_return_1d` | float64 | 100.0% | 0.000 | 0.019 | Target return minus benchmark return over the session at or after publication. The label. Deliberately forward-looking, the only column here that is. |
| `label_direction` | int64 | 100.0% | 0.018 | 1.000 | sign(abnormal_return_1d) as -1 / 0 / 1. 0 means the return was missing or exactly flat, so it is not a third class to predict. |
| `momentum_1d` | float64 | 100.0% | 0.002 | 0.017 | Cumulative return over the trading day before publication. |
| `momentum_5d` | float64 | 100.0% | 0.011 | 0.038 | Cumulative return over the 5 trading days before publication. |
| `momentum_20d` | float64 | 100.0% | 0.034 | 0.076 | Cumulative return over the 20 trading days before publication. |
| `volatility_20d` | float64 | 100.0% | 0.015 | 0.005 | Std dev of daily returns over the 20 trading days before publication. |
| `beta_20d` | float64 | 100.0% | 0.740 | 0.421 | Rolling 20-day beta of the target against the benchmark, before publication. |
| `relative_volume_20d` | float64 | 100.0% | 1.164 | 0.538 | Prior day's volume over its trailing 20-day median. |
| `daily_range_ratio_1d` | float64 | 100.0% | 0.022 | 0.010 | Prior day's high-low range, scaled by that day's close. |
| `days_to_earnings` | int64 | 100.0% | 40.284 | 27.354 | Calendar days to the next earnings date on or after publication. |
| `session` | object | 100.0% |  |  | Market session the article was published into: pre-market, market-hours, after-hours. |
| `news_volume` | int64 | 100.0% | 87.958 | 47.727 | Count of this ticker's own articles in the 3 days before publication. Scoped to one corpus, so it measures coverage intensity for this company rather than how busy the market was overall. |

![AAPL feature distributions](figures/AAPL_market_feature_distributions.png)
