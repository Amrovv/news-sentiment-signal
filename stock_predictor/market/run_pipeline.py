...
Run with `python -m stock_predictor.market.run_pipeline TICKER`, or
...
def load_market_inputs(ticker: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for path, hint in [
        (RAW_OHLCV_PATH, "python -m stock_predictor.market.prices " + ticker),
        (RAW_SCHEDULE_PATH, "python -m stock_predictor.market.prices " + ticker),
        (raw_earnings_path(ticker), "python -m stock_predictor.market.prices " + ticker),
    ]:
        if not path.exists():
