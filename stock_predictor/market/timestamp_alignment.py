...
def align_timestamp(pub_datetime: dt.datetime, schedule: pd.DataFrame) -> pd.Timestamp:
    ...
    if (pub_ts > pd.Timestamp(NEWS_END_DATE, tz="UTC")) or (pub_ts < pd.Timestamp(NEWS_START_DATE, tz="UTC")):
         raise ValueError(
            f"Publication datetime {pub_ts} is outside the publication timeframe"
        )
