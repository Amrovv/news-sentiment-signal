...
Run with `python -m stock_predictor.merge.run_pipeline TICKER`, or `--all`.
...
from stock_predictor.merge import integrity, leakage, report
...
def load_tables(ticker: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for path, hint in [
        (text_path, f"python -m stock_predictor.text.run_pipeline {ticker}"),
        (market_path, f"python -m stock_predictor.market.run_pipeline {ticker}"),
    ]:
        if not path.exists():
