...
# Tickers
PRIMARY_TICKER = "TSLA"  # default target for the text modules' ticker parameter
...
# The per-ticker raw pull, before the scrape stage resolves source/time/body.
# raw_articles_path()/processed_articles_path() are the forms the fetch
# package's own CLIs (finnhub_pull.py, scrape.py) read and write.
...
# processed_body. Written by news_sentiment.fetch.scrape, ~530MB, kept out
...
# Resolved once per process by news_sentiment.text.device, which also falls
