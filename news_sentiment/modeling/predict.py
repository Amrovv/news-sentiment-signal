...
Run with `python -m news_sentiment.modeling.predict INPUT.parquet [--out OUT]`.
...
from news_sentiment.modeling.train import MODEL_PATH
...
def load_model(path=MODEL_PATH) -> dict:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"No model at {path}. Train it first: python -m news_sentiment.modeling.train"
        )
